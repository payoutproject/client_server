...
from apps.public.serializers import ManageSerializer,QrcodeModelSerializer,WechatHelperModelSerializer,QrTypeModelSerializer
import hashlib
...
class PublicAPIView(viewsets.ViewSet):
            ...
            return { "data" : {"router" : [
                ...
                },
                {
                    ...
                    "children": [
                        ...
                        {"path": '/sysparams', "component": "sysparams", "name": '系统参数'}
                    ]
                },
                {
                    "path": '/cqmanage',
    ...
    @list_route(methods=['GET'])
    @Core_connector()
    def get_sysname(self,request):

        return {"data":{ "sysname":Sysparam.objects.get().sysname}}


    #修改系统名称
    @list_route(methods=['POST'])
    @Core_connector(transaction=True)
    def upd_sysname(self,request):

        obj = Sysparam.objects.get()
        obj.sysname = request.data_format["sysname"]
        obj.save()
        return None
    ...
    @list_route(methods=['GET'])
    @Core_connector(pagination=True)
    def qrtype_query(self,request):

        return {"data":QrTypeModelSerializer(QrType.objects.filter(),many=True).data}

    #新增二维码类型
    @list_route(methods=['POST'])
    @Core_connector(transaction=True)
    def qrtype_add(self,request):

        if not request.data_format.get("name"):
            raise PubErrorCustom("请输入二维码名称")

        QrType.objects.create(**{
            "qrtype": request.data_format["name"]
        })
        return None
    ...
    @list_route(methods=['POST'])
    @Core_connector(transaction=True)
    def qrtype_upd(self, request):

        if not request.data_format.get("id"):
            raise PubErrorCustom("id不能为空!")
        if not request.data_format.get("qrtype"):
            raise PubErrorCustom("请输入二维码名称")
        try:
            obj = QrType.objects.get(id=request.data_format.get("id"))
        except QrType.DoesNotExist:
            raise PubErrorCustom("无此二维码类型")

        obj.qrtype = request.data_format["qrtype"]
        obj.save()
        return None
    ...
    @list_route(methods=['POST'])
    @Core_connector(transaction=True)
    def qrtype_del(self, request):

        QrType.objects.filter(id=request.data_format.get("id")).delete()
        return None
